...
import MaskRevisionModal from "../components/generation/MaskRevisionModal";
import VersionGrid from "../components/generation/VersionGrid";
import ViewSlot from "../components/generation/ViewSlot";
import { VIEW_LABELS } from "../components/generation/labels";
...
export default function GenerationPanel() {
  ...
  async function handleFactoryPdf() {
    try {
      ...
      alert(`导出工厂 PDF 失败: ${getErrorMessage(error)}`);
    }
  }
  ...
  async function handlePackage() {
    try {
      ...
      alert(`导出资料包失败: ${getErrorMessage(error)}`);
    }
  }
  ...
  async function handleMaskedRevision(generation: Generation, maskDataUrl: string, prompt: string) {
    ...
    try {
      ...
      alert(`局部修版失败: ${getErrorMessage(error)}`);
    } finally {
      setGenerating(false);
    }
  }
