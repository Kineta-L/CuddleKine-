...
def save_settings(patch: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in patch.items():
        if key not in current or value is None:
            continue
        if key in ("openai_api_key", "replicate_api_token"):
...
def _normalize_secret(value: str) -> str:
    ...
    match = re.search(r"(r8_[A-Za-z0-9]+|sk-[A-Za-z0-9_\-]+)", value)
    return match.group(1) if match else value


def get_default_provider() -> str:
    provider = str(load_settings().get("default_provider") or GENERATION_PROVIDER)
    return provider if provider in {"comfyui", "openai", "replicate", "mock"} else GENERATION_PROVIDER
